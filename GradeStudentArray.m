function [ArrayStudents] = GradeStudentArray(ArrayStudents, assignmentSolution, studentResultsFileWriter)
%
% [ArrayStudents] = GradeStudentArray(ArrayStudents, ...
...
for I = 1:length(ArrayStudents)
    ...
    %Set Userpath to studentDirectory so student can reference their own
    %functions in other functions
    studentDirectory = fullfile(studentResultsFileWriter.studentPath, ArrayStudents(I).studentFolderName, 'Submission attachment(s)');
    ...
    for J = 1:length(studentSubmittedProblems)
        ...
        if studentSubmittedProblem.isProblemSubmitted
            ...
            if isNarginIncorrect && ~strcmpi(problemSolution.problemType, 'plot_visual')
                ...
                problemResults.problemStatus = 'Error: INCORRECT NUMBER OF INPUT OR OUTPUT PARAMS';
                ...
                for K = 1:length(problemTestCases)
                    ...
                    if strcmpi(problemSolution.problemType, 'function')
                        ...
                        userpath(studentDirectory);
                        rng('default');       %newest seed version
                        % rand('twister',0);    %original stationary seed
                    ...
                    elseif strcmpi(problemSolution.problemType, 'recursive')
                        
                        % Initialize the rand seed for student function.
                        userpath(studentDirectory);
                        rng('default');       %newest seed version
                        % rand('twister',0);    %original stationary seed
                        student_output = get_recursion_output(studentSubmittedProblem.problemFunctionHandle, problemTestCase);
                        userpath('reset')
                        ...
                        rng('default');       %newest seed version
                        % rand('twister',0);    %original stationary seed
                        correct_output = get_recursion_output(problemSolutionFunction, problemTestCase);
                        userpath('reset')
                        
                    elseif strcmpi(problemSolution.problemType, 'script')
                        
                        % changed 8.30.12 to false // to use script function
                        ...
                        if flagPlotVisual
                            ...
                            solutionPath = evalin('base', 'solutionPath');
                            problemSolutionFunction = fullfile(solutionPath, studentSubmittedProblem.problemFileName);
                            clear solutionPath
                    ...
                    if strcmpi(problemSolution.problemType, 'plot_visual') || ...
                        ...
                        testCaseResults.studentOutput = {'Visually graded'};
    ...
    [studentGrade] = UpdateGradebook(ArrayStudents(I), studentResultsFileWriter, 'grades.csv');
...
function [studentGrade] = UpdateGradebook(student, studentResultsFileWriter, filename)

% Put an assertion that the number of students in the gradebook equals the
% number of students in the graded student array.

fgradebook = fullfile(studentResultsFileWriter.studentPath, filename);
...
for i = 1:length(student_output)
    ...
    elseif isa(correct_output{i}, 'function_handle')
        ...
        testCase.testCaseInput = testCase.testCaseInput{end};
        
        try
            temp_student_output = get_function_output(student_output{i}, testCase);
        catch err
            temp_student_output = ['Error: ' err.message];
        end
...
%   See TestCase.m, properties: testCaseFlagOutputConfig,
%   testCaseOutputConfig
%
% To allow formatting function outputs for comparison in following cases:
%
% 0. Multiple evaluations of single output variables. (last added)
...
%Store original output argument; overwritten at end of this function
studentOutputConfig = student_output;
correctOutputConfig = correct_output;


% make sure nargout is in bounds for testCaseOutputConfig
if max([outFormTemp{1:3:end}]) > length(correct_output)
    error(['\nNumber of output arguments supposed by testCaseOutputConfig\n\t', ...
        'exceeds the number of output variables'])
end

% ATTEMPT TO MODIFY FOR n OUTPUT ARGUMENT COMPARISON
for indOutArg = 1:length(outFormTemp)/3
    
    %Evaluated output-configured variables
    outTemp = cell(1,2);
    
    % Which output argument to evaluate
    outArg = outFormTemp{3*(indOutArg-1)+1};
    studentOutTemp = studentOutputConfig{outArg};
    correctOutTemp = correctOutputConfig{outArg};
    ...
    if isa(studentOutTemp, 'char') & ~strcmpi(studentOutTemp(1:5),'Error')
        outTemp{1} = studentOutTemp;
        outTemp{2} = correctOutTemp;
    end
    ...
    fcnHan = outFormTemp{3*(indOutArg-1)+2};
    indStrct = outFormTemp{3*indOutArg};
    isMT_fcnHan = isempty(fcnHan);
    isMT_indStrct = isempty(indStrct);
    if (isMT_fcnHan+isMT_indStrct) == 2             % output not modified; next loop
        student_output{indOutArg} = studentOutTemp;
        correct_output{indOutArg} = correctOutTemp;
        continue;
    ...
    % thru student_output, correct_output
    for ind = 1:2
    ...
    %Move temporary outputs back to regular output ...
    %  Actual outputs stored in studentOutputConfig, correctOutputConfig
    %  for next loop evaluation
    %Note: char were not changed because sill may contain error from evaluation
    if isempty(outTemp{1})
        student_output{indOutArg} = studentOutTemp;     %error
    else
        student_output{indOutArg} = outTemp{1};
    end
    if isempty(outTemp{2})
        correct_output{indOutArg} = correctOutTemp;     %error
    else
        correct_output{indOutArg} = outTemp{2};
    end
end


% Print to screen to show outputs in log:
fprintf('\nFormatted student output:\n');
for ind = 1:length(outFormTemp)/3
    fprintf('\t');
    cprintf(student_output{ind});
end

fprintf('Formatted correct output:\n');
for ind = 1:length(outFormTemp)/3
    fprintf('\t');
    cprintf(correct_output{ind});
end
...
function [output] = get_recursion_output(fun, testCase)

% Evaluate output
[output] = Eval_output(testCase, fun, 'recursive');
...
%modified to allow for nargin since nInputs returns the maxNargin
nargInputs = min(length(inputTemp), nInputs);

for i = 1:nargInputs
    S = [S, input_name, '{',num2str(i),'}'];
    if i < nargInputs, S = [S, ', ']; end
end
S = [S, ');'];
...
if strcmpi(problemType, 'recursive')
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    profile off;
    profile clear;
    profile on -history
...
if strcmpi(problemType, 'recursive')
    ...
    profile off;
    stats = profile('info');
    %create cellstr of called functions, builtins, etc.
    callList = cellstr(strvcat(stats.FunctionTable.FunctionName));
    %match the name of our recursive function
    indFcn = strmatch( callList{end} , callList);
    %flag reveals if recursive
    isRecursive = any([stats.FunctionTable.IsRecursive]);
    %number of times called
    numCalls = stats.FunctionTable(indFcn).NumCalls;
    
    %assert "Error." output string for using recursive function
    if ~isRecursive && inputTemp{1} > 2
        output{1} = 'Error: Function is not recursive';
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
...
% shortFilenameFunction = [shortFilenameScript, '_func'];   %overwritten from above
